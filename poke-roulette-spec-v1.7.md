...
| **Persistence**     | `localStorage` — all lists and settings survive page refresh. When in an online room, the server is the source of truth for list state.                                                                                          |
| **Online mode**     | Optional multiplayer via the Sync API (see Section 4.4). Users join named rooms; all participants share live list state over SSE. Auth via username + password, JWT stored in memory.                                            |
...
### 2.3 In-memory auth state

Auth state is never written to `localStorage`. It lives only in memory for the duration of the page session.

| Field          | Type             | Contents                                              |
|----------------|------------------|-------------------------------------------------------|
| `authToken`    | string \| null   | Current JWT. Cleared on sign-out or page close.       |
| `authUsername` | string \| null   | Authenticated username.                               |
| `currentRoom`  | object \| null   | `{code, role, members}` when in a room; null otherwise.|
...
- Resolve removes all unselected copies. Pokémon with no selection are left unchanged.

---

## 4. Sidebar

The sidebar is fixed to the right side of the viewport. It contains a tab bar at the top and a scrollable content area below.
...
| Online tab  | ⊕     | Switch to the Online view              |
...
Accessible via the ⊕ tab. Manages authentication and room membership.

The view renders one of four sub-sections depending on auth/room state:

| State                      | Sub-section shown    |
|----------------------------|----------------------|
| Logged out                 | Auth section         |
| Logged in, not in a room   | Room section         |
| In a room                  | Active room section  |

#### Auth Section

Shown when not logged in. Contains two sub-tabs: **Sign In** and **Register**.
...
- Password field
- "Register" button
- Error message area (shown on failure, e.g. username taken)

On success (either tab), the returned JWT is stored in memory as `authToken`, username stored as `authUsername`, and the view transitions to the Room section.

A "Change password" link is shown below the signed-in state (Room section) — see below.

#### Room Section

Shown when logged in but not in a room. Contains:

- Greeting: "Signed in as **{username}**"
- **Create Room** sub-section:
  - Text input for room code with live sanitization (see Section 4.4 — Room Code Sanitization) and inline format validation
  - "Create" button (disabled until code is valid)
- **Join Room** sub-section:
  - Text input for room code
  - "Join" button
- **Change password** link (opens a modal with current password, new password fields and a confirm button)
- **Sign out** button (clears `authToken`, `authUsername`, `currentRoom` from memory)

#### Active Room Section

Shown when in a room. Contains:

- Room code (displayed), with a copy-to-clipboard button
- SSE connection indicator dot: green (connected) / amber (reconnecting)
- Role badge for the current user
- Member list: each row shows username and role badge. For non-self members, host and moderator callers see a `…` button that opens a role assignment menu showing only roles they may assign.
- **Leave Room** button (disabled for host when other members are present; tooltip: "Transfer host role before leaving")
- **Close Room** button (host only)

#### Room Code Sanitization

Applied live as the user types in the Create Room input:
...
Sanitization is display-only during typing. Final validation (length 3–32, no leading/trailing/double hyphens) gates the Create button.

#### Authentication

- Tokens are stored in memory only (`authToken` variable). Never written to `localStorage` or `sessionStorage`.
- All API requests include `Authorization: Bearer {authToken}`.
- On every API response, if the `X-Refreshed-Token` header is present, `authToken` is replaced with the new value.
- On page close/reload, auth state is lost and the user must sign in again.

#### Sync Behavior

- On room creation: current local lists are pushed to the server as the initial state.
- On room join: server list state overwrites local `localStorage` lists.
- Every local list mutation (`saveLists`) pushes the full state to the server when in a room (last write wins). Local save still happens as normal.
- SSE `lists_updated` events from other users overwrite local state and re-render. Own updates are ignored (identified by `updated_by` matching `authUsername`) to avoid flicker.
- Config, hint settings, and generation filters remain local and are not synced.
- On `room_closed` SSE event: close the SSE connection, clear `currentRoom`, show a notification "Room was closed by {username}", return to the Room section.
- On SSE reconnect: the server sends a fresh `connected` event with full state; client overwrites local lists from it.

#### SSE Connection Management

The client uses the native `EventSource` API. On disconnect, `EventSource` reconnects automatically. The connection indicator shows amber while reconnecting and green once the `connected` event is received.

The `EventSource` is closed when:
- The user leaves or the room is closed
- The user signs out
...
- Displaying profile pictures or display names (username only)
- Syncing config, hints, or generation filter settings (these remain local per user)
- Room password protection or spectator-only mode
